...
function myUtilities(newContainer,newImagePanel, newFolders){
    ...
    this.tagClick = function(event){
        ...
        else if(floatDivBehavior == "DrillDown")
        {
            $("#"+container+" .floatDiv").animate({width:"100%",opacity:1});
            $("#"+container+" .floatTagD").css("visibility","hidden");
        }
    ...
    this.setupDrawer = function(){
        ...
        floatDivBehavior = "Drawer";  
    };
    
    this.setupDrillDown = function(){
        var i;
        $("#"+this.container).append("<div class='floatDiv'></div>");
        $("#"+this.container).append("<div class='floatTagD'></div>");
        for(i=0; i<this.folders.length; i++)
        ...
        $("#"+this.container+" .floatDiv .selectableDiv").click({param1:this.imp, param2:".floatDiv"},this.selectFolder);    
        floatDivBehavior = "DrillDown";
        
        $("#"+this.container+" .floatDiv .selectableDiv").click({param1:this.container},this.animateMenu);
    };
    
    this.animateMenu = function(event){
        var container = event.data.param1;
        $("#"+container+" .floatDiv").animate({width:"0%"},550, function(){
            $("#"+container+" .floatTagD").css("visibility","visible");
            $("#"+container+" .floatDiv").css("opacity",0);
            $("#"+container+" .floatTagD").css("left",-10);
        });        
    };
